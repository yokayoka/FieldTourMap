...
## Phase 6: エクセルブック（.xlsx）形式でのプロジェクト入出力（Requirement 17）

Phase 5完了後、ユーザーからの追加要望により、Googleアカウントを持たない主催者向けに、Admin Config Toolから`.xlsx`ワークブック形式でプロジェクトをダウンロード・アップロードできる機能（Requirement 17）を追加した。Googleスプレッドシート連携（Requirement 15）と同一のシート構成・列定義を用い、変換ロジック（GoogleSheetsRowMapping）を再利用する。参加者向けMap Viewerには実装しない。

- [ ] 32. 🔴 XlsxWorkbookAdapterの実装（TDD）
  - 新規npm依存としてSheetJSの`xlsx`パッケージ（`dependencies`）を追加
  - `admin-tool/src/services/xlsxWorkbookAdapter.ts`: `SheetsData`⇄`.xlsx`バイナリ（`ArrayBuffer`）の相互変換関数`sheetsToWorkbook`/`workbookToSheets`をTDD実装（往復一致テスト、想定シート名欠落時の異常系）
  - `admin-tool/src/`配下からのみ利用し、参加者向け`main.js`バンドルに影響しないことをビルド成果物で確認
  - _Requirements: 17_

- [ ] 33. 🔴 Admin Config Tool（レイヤー編集）への`.xlsx`連携統合
  - 「エクセルブックとしてダウンロード」「エクセルブックを読み込む」ボタンを追加し、`AdminLayerListStore`と連携（`Layers`単一シートのワークブック）
  - 既存の`validateLayerDefinition`での検証、失敗時も既存のJSON/Googleスプレッドシート機能に影響を与えないことをテストで確認
  - _Requirements: 17_

- [ ] 34. 🔴 Admin Config Tool（ツアー編集）への`.xlsx`連携統合
  - Task 33と同様のUIをツアー編集ページに追加し、`AdminTourStore`と連携（`Tours`/`POIs`/`Media`/`ReferencePapers`/`Routes`/`RoutePoints`の6シート、現在の「ツアーID」欄の値を対象に保存・読み込み）
  - 複数ツアーを含むワークブックから対象ツアーのみを読み込めることのテスト
  - _Requirements: 17_

- [ ] 35. 🔴 サンプルのエクセルブックファイル追加とドキュメント整備
  - `docs/sample-project/`に、既存のCSVサンプル（`Layers.csv`等7ファイル）と同じ内容をまとめた単一の`.xlsx`ワークブックファイルを追加
  - `docs/organizer-guide.md`に`.xlsx`のダウンロード・アップロード手順、サンプルワークブックの使い方を追記
  - _Requirements: 17_
